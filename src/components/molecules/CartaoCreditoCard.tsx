...
import { Text, StyleSheet, View, Modal, TouchableOpacity, FlatList, ActivityIndicator } from 'react-native'
...
type ParcelaApi = {
  ...
  despesa: { id: string; descricao: string; valor: number | string; metodoPagamento: string; data: string }
}
...
const colorPorPercent = (percent: number) => {
  ...
  if (p < 33) {
    // Verde azulado suave a verde
    const r = Math.round(100 + (p / 33) * 80);       // 100 → 180
    const g = Math.round(200 + (p / 33) * 55);       // 200 → 255
    const b = Math.round(180 - (p / 33) * 100);      // 180 → 80
    return `rgb(${r}, ${g}, ${b})`;
  } 
  else if (p < 66) {
    // Verde a amarillo dorado
    const normalizedP = (p - 33) / 33;
    const r = Math.round(180 + normalizedP * 75);    // 180 → 255
    const g = Math.round(255 - normalizedP * 55);    // 255 → 200
    const b = Math.round(80 - normalizedP * 60);     // 80 → 20
    return `rgb(${r}, ${g}, ${b})`;
  } 
  else {
    // Amarillo dorado a rojo coral suave
    const normalizedP = (p - 66) / 34;
    const r = 255;                                   // Rojo máximo
    const g = Math.round(200 - normalizedP * 150);   // 200 → 50
    const b = Math.round(20 - normalizedP * 20);     // 20 → 0
    return `rgb(${r}, ${g}, ${b})`;
  }
};
...
export default function CartaoCreditoCard({ cartaoId }: { cartaoId: string }) {
  ...
  const normalizeDetalhe = (det: FaturaDetalhe): FaturaDetalhe => ({
    ...
    resumo: { restante: Number(det.resumo.restante), percentPago: Number(det.resumo.percentPago) }
  })
  ...
  const fetchFaturaMes = useCallback(async (m: number, a: number) => {
    if (m < 1) { m = 12; a -= 1 }
    if (m > 12) { m = 1; a += 1 }
    const k = key(m, a)
    ...
    setLoadingFatura(true)
    try {
      const r = await api.get(`/faturas/cartao/${cartaoId}/mes?mes=${m}&ano=${a}`)
      const det = normalizeDetalhe(r.data.data)
  ...
  if (loadingResumo && !resumo) {
    return <Card style={styles.card}><ActivityIndicator /></Card>
  }
  ...
  return (
    <>
      <Card style={styles.card}>        
        ...
        {resumo.conta && (
          <Text style={styles.sub}>{resumo.conta.conta} - {resumo.conta.bancoNome}</Text>
        )}

<View style={styles.progressOuter}>
  <View style={[styles.progressInner, { flex: progressFraction, backgroundColor: colorPorPercent(percentUsed) }]}>
  ...
  <View style={{ flex: 1 - progressFraction }} />
        ...
        <View style={styles.infoRow}>
          <Text style={styles.disponivel}>Disponível: {currency(resumo.available)}</Text>
          <Text style={styles.usado}>Usado: {currency(resumo.creditUsed)}</Text>
        </View>
        <Text style={styles.limite}>Limite: {currency(resumo.creditLimit)}</Text>
      ...
      <Modal visible={modalVisible} transparent animationType='fade' onRequestClose={() => setModalVisible(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            {loadingFatura && !detalhe && <ActivityIndicator size='large' color='#4a90e2' />}
            {detalhe && (
              <>
                <Text style={styles.modalTitle}>Fatura {mes}/{ano}</Text>
                <Text style={styles.resumo}>
                  Total: {currency(Number(detalhe.fatura.valorTotal))} | Pago: {currency(Number(detalhe.fatura.valorPago))} | Restante: {currency(detalhe.resumo.restante)} ({pct(detalhe.resumo.percentPago)})
                </Text>
                ...
                <FlatList
                  ...
                  ListEmptyComponent={<Text style={{ textAlign: 'center', color: '#666' }}>Sem parcelas</Text>}
                  renderItem={({ item }) => (
                    <View style={styles.detalheItem}>
                      <Text style={styles.detalheDescricao}>{item.numeroParcela}. {item.despesa.descricao}</Text>
                      <Text style={styles.detalheValor}>{currency(Number(item.valor))}</Text>
                    </View>
                  )}
                />

                <View style={styles.navBtns}>
                  <TouchableOpacity style={styles.navBtn} onPress={() => navegar(-1)}>
            ...
            {!loadingFatura && !detalhe && (
              <Text style={{ textAlign: 'center', color: '#555' }}>Nenhuma fatura disponível</Text>
            )}
          </View>
        </View>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  progressLabel: { color: '#fff', fontWeight: '600', fontSize: 12, paddingHorizontal: 6 },
  ...
  detalhesBtn: { marginTop: 10, borderRadius: 12, paddingVertical: 10, backgroundColor: '#4a90e2', alignItems: 'center' },
  ...
  resumo: { fontSize: 14, marginBottom: 12, color: "#555", textAlign: "center" },
  detalheItem: { flexDirection: "row", justifyContent: "space-between", paddingVertical: 8, borderBottomWidth: 0.3, borderBottomColor: "#ddd" },
  detalheDescricao: { fontSize: 14, color: "#333" },
  detalheValor: { fontSize: 14, fontWeight: "600", color: "#222" },
  navBtns: { flexDirection: "row", justifyContent: "space-between", marginTop: 14 },
  navBtn: { flex: 1, backgroundColor: "#f2f2f2", paddingVertical: 10, marginHorizontal: 5, borderRadius: 12, alignItems: "center" },
  navText: { fontWeight: "600", color: "#4a90e2" },
  fecharBtn: { marginTop: 16, backgroundColor: "#4a90e2", paddingVertical: 12, borderRadius: 16, alignItems: "center" },
})
